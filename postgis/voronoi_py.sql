CREATE OR REPLACE FUNCTION voronoi_py(table_name text,geom_col text) returns SETOF record as $$
...
CREATE OR REPLACE FUNCTION voronoi_py(table_name text) returns SETOF record as $$
  SELECT * from voronoi_py($1, 'the_geom') as (id integer,the_geom geometry);
$$ LANGUAGE SQL;
